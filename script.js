(() => {
  ...
  // V56 project intake controller -----------------------------------------
  // Restores the event bindings for every CTA marked data-open-project.
  ...
  const setProjectModal = (open, trigger = null) => {
    ...
    if (open) {
      setTimeout(() => {
        projectModal.querySelector("textarea, input, select, button")?.focus();
      }, 80);
    } else if (
      lastProjectTrigger &&
      typeof lastProjectTrigger.focus === "function"
    ) {
  ...
  projectOpeners.forEach(opener => {
    opener.addEventListener("click", event => {
      event.preventDefault();
      setProjectModal(true, opener);
    });
  });

  projectClosers.forEach(closer => {
    closer.addEventListener("click", () => setProjectModal(false));
  });
  ...
  addEventListener("keydown", event => {
    ...
    trapFocusWithin(projectModal, event);
  });
  ...
  projectForm?.addEventListener("submit", event => {
    ...
    const timeline = String(data.get("timeline") || "Not sure yet");
...
createFluidTrail({
  ...
  if (liquidStack && liquidCanvas && liquidTitle) {
    ...
    const drawTracked=(ctx,text,cx,lineTop,size,lineHeight,family,weight,style,spacing)=>{
      const track=Number.isFinite(spacing)?spacing:0;
      const cssLineHeight=Number.isFinite(lineHeight)?lineHeight:size;
      ...
      /*
       * Match the browser's compressed line box. The headline uses line-height
       * below 1, so drawing from textBaseline=top caused the dark copy to sit
       * lower than the real DOM letters. Canvas font metrics give us the same
       * baseline geometry the browser uses for the loaded display font.
       */
      ...
      const halfLeading=(cssLineHeight-fontBox)*.5;
      const baseline=lineTop+halfLeading+ascent;

      /*
       * Modern browsers support Canvas letterSpacing. It preserves normal
       * kerning/shaping and matches the DOM more accurately than drawing each
       * character separately. Keep a fallback for older WebKit.
       */
    ...
    const rebuild=()=>{
      ...
      /*
       * The H1 can intentionally be wider than the padded hero content on
       * phones. Force the liquid stack to the H1's rendered width before
       * measuring the WebGL canvas so both layers share one coordinate space.
       */
      liquidStack.style.width="";
      const titleRect=liquidTitle.getBoundingClientRect();
      liquidStack.style.width=`${Math.max(1,titleRect.width)}px`;
      ...
      liquidTitle.querySelectorAll(":scope > span").forEach(span=>{
        ...
        const lineHeightRaw=parseFloat(st.lineHeight);
        const lineHeight=(Number.isFinite(lineHeightRaw)?lineHeightRaw:parseFloat(st.fontSize)||100)*dpr;
