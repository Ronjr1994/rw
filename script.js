  ...
  function setupCrispRail(){
    ...
    const move = (instant = false) => {
      track.style.transition =
        instant || reducedRail.matches
          ? "none"
          : "transform 420ms cubic-bezier(.25,.8,.25,1)";

      track.style.transform = reducedRail.matches
        ? "none"
        : `translate3d(${-step() * index}px,0,0)`;
    };
    ...
    const interval = () => {
      if (innerWidth <= 680) return 1700;
      if (innerWidth <= 980) return 1850;
      return 2000;
    };
    ...
    /* Deliberately no mouseenter/mouseleave pause: always continuous. */

    crispRail.addEventListener("touchstart", event => {
    ...
    crispRail.addEventListener("touchend", event => {
    ...
    crispRail.addEventListener("touchcancel", () => {
      setTimeout(play, 250);
    }, {passive:true});
